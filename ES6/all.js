$(function () {
  ...
  let arr = new Array(10).fill(Object.create(null)).map(i => ({ num: Math.random() }))
  console.log(arr)
  let set = new Set(arr);
  console.log(set)
  ...
  * */
  ...
  //     let x = 1;
  // }
  // 上面代码如果使用var 替代let ,console.log 那一行就不会报错
  // 而是会输出 undefined ,因为变量提升到代码块的头部。
  // 这违反了变量先声明后使用的原则
  // 所以建议不再使用var 命令,而是使用let 命令取代

  // (2) 全局常量和线程安全
  // 在 let 和const之间,建议优先使用const,尤其是在全局环境,
  // 不应该设置变量,只应设置常量

  // const 好过 let 的原因:
  // 一. const 可以提醒阅读程序的人,这个变量不应该改变
  // 二. const 比较符合函数式编程思想,运算不改变值,只是新建值,而且这也有利于将来的分布式运算
  // 三. JavaScript 编译器会对const进行优化,多使用const,有利于提高程序的运行效率
  //     let 和 const 的本质区别,其实是编译器内部的处理不同
  ...
  // const [a,b,c] = [1,2,3];
  // const 声明常量还有两个好处:
  // 1. 阅读代码的人理科会意识到不应该修改这个值
  // 2. 防止无意间修改变量值所导致的错误
  // 所有的函数都应该设置为常量
  // 从长远来看,javascript 可能会有多线程的实现,
